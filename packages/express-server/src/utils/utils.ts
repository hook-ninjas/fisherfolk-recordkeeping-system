...
const isValidUserName = (username: string) => /^[a-zA-Z][a-zA-Z0-9_.]{5,}$/.test(username);

// password must have a digit and be atleast 8 characters long
const isValidPassword = (password: string) => /^(?=.*\d)[a-zA-Z\d]{7,}$/.test(password);

// function to encode file data to base64 encoded string
const convertToUri = (base64: string | Buffer, options: { type: string; format: string }) => {
  const { format, type } = options;
  const result = `data:${type}/${format};base64,${base64}`;
  return result;
};

export { nonNullArg, nullableList, isValidPassword, isValidUserName, convertToUri };
